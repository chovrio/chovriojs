...
export enum TokenType {
  ...
  /** @description 返回关键字 return */
  Return = 'Return',
  /** @description 导入关键字 import */
  Import = 'Import',
  /** @description 导出关键字 export */
  Export = 'Export',
  /** @description 默认关键字 default */
  Default = 'Default',
  /** @description 从...导入关键字 from */
  From = 'From',
  /** @description 重命名关键字 as */
  As = 'As',
  /** @description 星号 * */
  Asterisk = 'Asterisk'
}
...
export enum ScanMode {
  /** @description 正常模式，用于扫描普通的标识符、关键字和符号。 */
  Normal,
  /** @description 标识符模式，用于扫描变量名、函数名等标识符。 */
  Identifier,
  /** @description 字符串字面量模式，用于扫描字符串字面量。 */
  StringLiteral,
  /** @description 数字模式，用于扫描数字字面量。 */
  Number
}
...
export type Token = {
  /** @description 表示标记的类型 */
  type: TokenType;
  /** @description 标记的值，通常用于存储标识符、关键字、字符串字面量等的具体内容。 */
  value?: string;
  /** @description 标记在源代码中的起始位置（偏移量）。 */
  start: number;
  /** @description 标记在源代码中的结束位置（偏移量）。 */
  end: number;
  /** @description 标记的原始字符串表示。 */
  raw?: string;
};
...
export class Tokenizer {
  ...
  private _scanMode: ScanMode = ScanMode.Normal;
  ...
  tokenize(): Token[] {
    while (this._currentIndex < this._source.length) {
      ...
      // 3.判断是否是单字符 () {} . ;
      if (KNOWN_SINGLE_CHAR_TOKENS.has(<SingleCharTokens>currentChar)) {
        // * 字符特殊处理
        if (currentChar === '*') {
